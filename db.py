...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # Таблица личных сказок пользователя
        await db.execute("""
        ...
        """)
        # Пример заполнения готовых сказок (можно заменить на реальные данные)
        # await db.execute("INSERT OR IGNORE INTO skazki (text, audio_path, type) VALUES (?, ?, ?)",
        #                  ("Жил-был старик...", None, "text"))
        # await db.execute("INSERT OR IGNORE INTO skazki (text, audio_path, type) VALUES (?, ?, ?)",
        #                  ("В тридевятом царстве...", "path/to/audio.mp3", "audio"))
        await db.commit()
...
async def get_random_skazka(tale_type="text"):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT text, audio_path FROM skazki WHERE type = ? ORDER BY RANDOM() LIMIT 1",
                              (tale_type,)) as cursor:
            return await cursor.fetchone()
